...
export class SpotClient {
  ...
  constructor(
    ...
    this.requestWrapper = new RequestWrapper(
        ...
        { ...restClientOptions, timePath: restClientOptions.timePath || "spot/v1/time", recv_window: 5000 },
        requestOptions
    );
    return this;
  }
  ...
  submitOrder(params: NewSpotOrder) {
    return this.requestWrapper.post('spot/v1/order', params);
  }

  getOrder(params: SpotOrderQueryById) {
    return this.requestWrapper.get('spot/v1/order', params);
  }

  cancelOrder(params: SpotOrderQueryById) {
    return this.requestWrapper.delete('spot/v1/order', params);
  }
  ...
  cancelOrderBatch(params: {
    ...
    return this.requestWrapper.delete('spot/order/batch-cancel', {
      ...params,
      orderTypes,
    });
  }
  ...
  getOpenOrders(symbol?: string, orderId?: string, limit?: number) {
    return this.requestWrapper.get('spot/v1/open-orders', {
      symbol,
      orderId,
      limit,
    });
  ...
  getPastOrders(symbol?: string, orderId?: string, limit?: number) {
    return this.requestWrapper.get('spot/v1/history-orders', {
      symbol,
      orderId,
      limit,
    });
  ...
  getBalances() {
    return this.requestWrapper.get('spot/v1/account');
  }
